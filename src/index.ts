import { AbstractConnectorArguments, ConnectorUpdate } from '@cheeseswapfinance/web3-react-types'
...
export class InjectedConnector extends AbstractConnector {
  ...
  public async activate(): Promise<ConnectorUpdate> {
    ...
    if ((window.ethereum as any).isMetaMask) {
      ;(window.ethereum as any).autoRefreshOnChainChanged = false
    }

    // try to activate + get account via eth_requestAccounts using request (modern EIP-1193)
    let account
    try {
      // Use the modern request method instead of request
